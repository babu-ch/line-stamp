...
- **フォントの採否はユーザー判断・ユーザー責任**: スタンプは販売物。Claude はフォントを「商用 OK」と**断定しない**。候補ごとに一次配布元 URL とライセンス要点を提示し、採否はユーザーが決める。フォント比較画像 (`font-test/comparison.png`) を組む前に、各候補のライセンス情報をユーザーに提示すること。Apple / SCREEN / フォント作者の解釈は変わりうるため、Claude の過去ガイダンスや過去採用実績を「将来も OK」の根拠にしない（過去 OK だったから今 OK、ではない）。クレジット表記が必要な場合は `style.md` か manifest にメモを残す。
  - **明確に NG（Claude が候補から外して良い）**: 商用利用に追加ライセンス購入が必須なもの（モリサワ / フォントワークス等の有償フォント）
  - **要ユーザー判断**: 上記以外すべて。フリーフォント (`851_kanaA/B`, `cinecaption`, `huifont` 等) も OS 同梱フォント (Hiragino 等) も、Claude は配布元情報を提示するだけ。採否決定の最終責任はユーザー側
