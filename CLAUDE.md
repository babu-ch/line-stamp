...
- **フォントは商用利用 OK のもののみ採用**: スタンプは販売物なので、一次配布元のライセンスで商用利用が許諾されているフォントを選ぶ。フォント比較画像 (`font-test/comparison.png`) に並べる前に各候補のライセンスを必ず確認すること。クレジット表記が必要なフォントは `style.md` か manifest にメモを残す。
  - **OK**: フリー商用 OK 系（例: `851_kanaA/B`, `cinecaption`, `huifont` 等。規約により再配布禁止・自作主張禁止・クレジット推奨などの条件あり、各々確認）
  - **OK**: macOS 同梱 **Hiragino** 系。Apple サポートの公式見解として「フォントファイル自体の配布・販売以外は OS X 付属フォント全て商用利用可、書類提出不要」。LINE スタンプ焼き込み販売は実績あり (`eco-mexico-sdgs`, `lowpoly-ferret-daily`)
  - **NG**: 商用ライセンスが追加購入必須のもの（モリサワ / フォントワークス等の有償フォント）。候補に並べない
